...
let ages2 = data1.filter(isDog).map(dogYears).reduce(sumOfAges);
...
let minutes = splitTime[1].includes('AM') ? splitTime[1].replace('AM', '') : splitTime[1].replace('PM', '');

splitTime[1].includes('AM')
  ? console.log(`Time is 0${hours}:${minutes}`)
